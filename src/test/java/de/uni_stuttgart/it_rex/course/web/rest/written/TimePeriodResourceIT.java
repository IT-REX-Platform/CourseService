...
public class TimePeriodResourceIT {
    ...
    void createTimePeriod() throws Exception {
        final int databaseSizeBeforeCreate =
    ...
    @Test
    @Transactional
    void getAllTimePeriods() throws Exception {
        ...
        restTimePeriodMockMvc.perform(get("/api/timeperiods?sort=id,desc"))
            ...
            .andExpect(jsonPath("$.[*].id")
                .value(hasItem(timePeriod.getId().toString())))
            .andExpect(jsonPath("$.[*].startDate")
                .value(hasItem(timePeriod.getStartDate().toString())))
            .andExpect(jsonPath("$.[*].endDate")
                .value(hasItem(timePeriod.getEndDate().toString())))
            .andExpect(jsonPath("$.[*].courseId")
                .value(hasItem(timePeriod.getCourse().getId().toString())));
    }

    @Test
    @Transactional
    void getTimePeriod() throws Exception {
        // Initialize the database
        ...
        // Get the TimePeriod
        restTimePeriodMockMvc
            .perform(get("/api/timeperiods/{id}", timePeriod.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.id").value(timePeriod.getId().toString()))
            .andExpect(jsonPath("$.startDate")
                .value(timePeriod.getStartDate().toString()))
            .andExpect(
                jsonPath("$.endDate").value(timePeriod.getEndDate().toString()))
            .andExpect(jsonPath("$.courseId")
                .value(timePeriod.getCourse().getId().toString()));
    }

    @Test
    @Transactional
    void getNonExistingTimePeriod() throws Exception {
        // Get the timePeriod
        restTimePeriodMockMvc
            .perform(get("/api/timeperiods/{id}", NON_EXISTING_ID))
            .andExpect(status().isNotFound());
    }

    @Test
    @Transactional
    void patchNonExistingTimePeriod() throws Exception {
    ...
    @Test
    void deleteTimePeriod() throws Exception {
        // Initialize the database
        final TimePeriod timePeriod = TimePeriodUtil.createTimePeriod();
        timePeriod.setCourse(THE_COURSE);
        timePeriodRepository.saveAndFlush(timePeriod);

        final int databaseSizeBeforeDelete = timePeriodRepository.findAll().size();

        // Delete the course
        restTimePeriodMockMvc.perform(
            delete("/api/timeperiods/{id}", timePeriod.getId()).with(csrf())
    ...
    void patchTimePeriodWithoutId() {
        ...
        Exception e = assertThrows(BadRequestAlertException.class,
            () -> timePeriodResource
                .patchTimePeriod(timePeriodMapper.toDTO(toUpdate)));
