...
public class CourseResourceIT {
    ...
    void createCourse() throws Exception {
        ...
        CourseDTO testCourseDTO =
            courseMapper.toDTO(courseList.get(courseList.size() - 1));
    ...
    void getAllCourses() throws Exception {
        ...
        restCourseMockMvc.perform(get("/api/courses?sort=id,desc"))
            ...
            .andExpect(
                jsonPath("$.[*].id").value(hasItem(course.getId().toString())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(course.getName())))
            .andExpect(jsonPath("$.[*].startDate")
                .value(hasItem(course.getStartDate().toString())))
            .andExpect(jsonPath("$.[*].endDate")
                .value(hasItem(course.getEndDate().toString())))
            .andExpect(jsonPath("$.[*].remainActiveOffset")
                .value(hasItem(course.getRemainActiveOffset())))
            .andExpect(jsonPath("$.[*].maxFoodSum")
                .value(hasItem(course.getMaxFoodSum())))
            .andExpect(jsonPath("$.[*].courseDescription")
                .value(hasItem(course.getCourseDescription())))
            .andExpect(jsonPath("$.[*].publishState")
                .value(hasItem(course.getPublishState().toString())));
    }
    ...
    void getCourse() throws Exception {
        ...
        restCourseMockMvc.perform(get("/api/courses/{id}", course.getId()))
            ...
            .andExpect(
                jsonPath("$.startDate").value(course.getStartDate().toString()))
            .andExpect(
                jsonPath("$.endDate").value(course.getEndDate().toString()))
            .andExpect(jsonPath("$.remainActiveOffset")
                .value(course.getRemainActiveOffset()))
            .andExpect(jsonPath("$.maxFoodSum").value(course.getMaxFoodSum()))
            .andExpect(jsonPath("$.courseDescription")
                .value(course.getCourseDescription()))
            .andExpect(jsonPath("$.publishState")
                .value(course.getPublishState().toString()));
    }
    ...
    void patchCourse() throws Exception {
        ...
        restCourseMockMvc.perform(patch("/api/courses").with(csrf())
            ...
            .andExpect(jsonPath("$.startDate")
                .value(expected.getStartDate().toString()))
            .andExpect(
                jsonPath("$.endDate").value(expected.getEndDate().toString()))
            .andExpect(jsonPath("$.remainActiveOffset")
                .value(expected.getRemainActiveOffset()))
            .andExpect(jsonPath("$.maxFoodSum").value(expected.getMaxFoodSum()))
            .andExpect(jsonPath("$.courseDescription")
                .value(expected.getCourseDescription()))
            .andExpect(jsonPath("$.publishState")
                .value(expected.getPublishState().toString()));
    ...
    void deleteCourse() throws Exception {
        ...
        // Delete the course
        restCourseMockMvc
            .perform(delete("/api/courses/{id}", course.getId()).with(csrf())
    ...
    void patchCourseWithoutId() {
        ...
        assertThrows(BadRequestAlertException.class,
            () -> courseResource.patchCourse(courseMapper.toDTO(toUpdate)));
    }
}
