...
public class ChapterResourceIT {
    ...
    void getAllChapters() throws Exception {
        ...
        restChapterMockMvc.perform(get("/api/chapters?sort=id,desc"))
            ...
            .andExpect(
                jsonPath("$.[*].id").value(hasItem(chapter.getId().toString())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(chapter.getName())))
            .andExpect(jsonPath("$.[*].chapterNumber")
                .value(chapter.getChapterNumber()))
            .andExpect(jsonPath("$.[*].courseId")
                .value(hasItem(chapter.getCourse().getId().toString())));
    }
    ...
    void getChapter() throws Exception {
        ...
        restChapterMockMvc.perform(get("/api/chapters/{id}", chapter.getId()))
            ...
            .andExpect(
                jsonPath("$.chapterNumber").value(chapter.getChapterNumber()))
            .andExpect(jsonPath("$.courseId")
                .value(chapter.getCourse().getId().toString()));
    }
    ...
    void deleteChapter() throws Exception {
        ...
        final int databaseSizeBeforeDelete = chapterRepository.findAll().size();

        // Delete the course
        restChapterMockMvc
            .perform(delete("/api/chapters/{id}", chapter.getId()).with(csrf())
    ...
    void patchChapterWithoutId() {
        ...
        Exception e = assertThrows(BadRequestAlertException.class,
            () -> chapterResource.patchChapter(chapterMapper.toDTO(toUpdate)));
